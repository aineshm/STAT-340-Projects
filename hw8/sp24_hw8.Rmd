...
plot(hp, mpg, main="Scatter plot of Horsepower vs MPG",
...
Based on the scatter plot, we  observe the relationship between hp and mpg. Since points tend to decrease as horsepower increases, it suggests a negative correlation, indicating that vehicles with higher horsepower tend to have lower fuel efficiency.
...
# Initialize a vector to store the permutation correlations
...
for (i in 1:n_perm) {
  ...
  perm_corrs[i] <- cor(hp, shuffled_mpg)
}
...
hist(perm_corrs, breaks=30, main="Permutation Test for Correlation",
...
In this permutation test, we randomly shuffle the mpg values while keeping the hp values fixed, then calculate the correlation coefficient for each permutation. This process is repeated 1000 times to create a distribution of correlation coefficients under the null hypothesis. We then compare the observed correlation coefficient with this distribution to calculate the p-value.

A low p-value (0 here) indicates that the observed correlation is significantly different from what would be expected under the null hypothesis of no correlation, leading us to reject the null hypothesis.
...
plot(catamaran$litter, catamaran$spending,
     main = "Scatter plot of Spending vs. Cat Litter Purchased",
     xlab = "Cat Litter Purchased (pounds)",
     ylab = "Spending on Catamaran Products (dollars)",
     pch = 19, col = "blue")
```

***

By observing the scatter plot, we are be able to discern whether there's a linear relationship between the amount of cat litter purchased and the total spending. Since there appears to be a straight-line trend, we can estimate the slope visually.The slope is approximately 30/25 or 6/5.
...
cat_intercept_hat <- unname(coef(model)[1])
cat_slope_hat <- unname(coef(model)[2])
...
The slope cat_slope_hat represents the increase in spending for each additional pound of cat litter, which is about 1.60
...
For a three-pound unit, we multiply the slope by 3 to get the increase in spending. So it would be cat_slope_hat * 3 or approximately 4.80 (using the rounded value from above)
...
For a twenty-pound unit, we multiply the slope by 3 to get the increase in spending. So it would be cat_slope_hat * 20 or approximately 32.0 (using the rounded value from above)
...
conf_int <- confint(model, 'litter', level = 0.95)
...
The Q-Q plot should show if the residuals deviate significantly from a normal distribution.
Based on the 95% confidence interval, we can determine if the null hypothesis that the slope (beta_1) is 0 should be rejected or not. Since 0 is within the confidence interval, we do not have enough evidence to reject the null hypothesis.
...
***
The length of the vector y will be the same as x and eps, which is n. The intercept (\(\beta_0\)) in this linear model is -1, and the slope (\(\beta_1\)) is 0.5.
***
...
generate_linear_data <- function(n, sigma2) {
  ...
  eps <- rnorm(n, mean = 0, sd = sqrt(sigma2))  # error term
  y <- -1 + 0.5 * x + eps  # response
  data.frame(predictors = x, responses = y)  # data frame
}
...
# Plot
plot(data$predictors, data$responses, main = "Scatterplot of Y vs. X",
     xlab = "X (Predictors)", ylab = "Y (Responses)", pch = 19, col = "blue")

```

***

We should observe a linear trend, though with some scatter due to the noise. The slope should appear to be about 0.5, and the intercept should seem close to -1.
...
data_high_noise <- generate_linear_data(100, 1)

# Plot
plot(data_high_noise$predictors, data_high_noise$responses, main = "High Noise",
     xlab = "X (Predictors)", ylab = "Y (Responses)", pch = 19, col = "red")

```

***

With higher noise (sigma2 = 1), the data points should be more spread out, making the linear trend less clear.
...
# Generate data with lower noise
data_low_noise <- generate_linear_data(100, 0.1)

# Plot
plot(data_low_noise$predictors, data_low_noise$responses, main = "Low Noise",
     xlab = "X (Predictors)", ylab = "Y (Responses)", pch = 19, col = "green")

```

***

With lower noise (sigma2 = 0.1), the data points should be closer to the line, making the linear relationship more apparent.
...
generate_and_estimate <- function(n, sigma2) {
  ...
  beta1hat <- unname(coef(model)[2])  # extracting slope estimate
  beta1hat
}
```
...
# Assuming the function definitions from previous parts are available and used here
sigma2_values <- seq(0.25, 4, by = 0.25)
variances <- sapply(sigma2_values, function(s) estimate_beta1hat_variance(100, s, 1000))

# Plot
plot(sigma2_values, variances, type = "b", col = "blue", pch = 19,
     main = "Variance of Estimator vs. Noise Level",
     xlab = "Noise Level (sigma2)", ylab = "Variance of Beta1 Hat")
...
The plot should show that as the noise level (\(\sigma^2\) increases, the variance of our estimator \(\hat{\beta}_1\) also increases. This aligns with the mathematical expectation that the variance of \(\hat{\beta}_1\) is directly proportional to \(\sigma^2\).
