...
```{r}
ans = pnorm(7, mean = 5, sd = 4) - pnorm(4, mean = 5, sd = 4)
ans
```
...
> probability of getting a 6 is 1/9

c. Give **two examples** of pairs of events that are mutually exclusive and explain why for each.

> Mutually exclusive events are those where the occurrence of one event precludes the occurrence of the other. For instance, in coin tossing, getting heads and getting tails are mutually exclusive outcomes. Similarly, rolling a die yields mutually exclusive events like getting an even or odd number.


d. Give **two examples** of pairs of events that are independent and explain why for each.

> Since independent events occur when the outcome of one event does not affect the probability of the other, the following examples cover pairs of independent events. Drawing a red card and drawing a face card from a deck of cards are independent events. Likewise, flipping a coin and rolling a die to get specific outcomes represent independent events since the results of one action do not influence the other's probabilities.
